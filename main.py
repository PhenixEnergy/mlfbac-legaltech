...
def convert_command(args) -> int:
    ...
    if hasattr(args, 'input') and args.input:
        script_args.append(args.input)  # positional argument
    if hasattr(args, 'output') and args.output:
        script_args.extend(['-o', args.output])  # output argument
    
    # Determine conversion direction based on file extensions
    if args.input.endswith('.json'):
        script_args.append('--to-jsonl')
    elif args.input.endswith('.jsonl'):
        script_args.append('--to-json')
...
def segment_command(args) -> int:
    ...
    if args.strategy == "training":
        ...
        # For now, use enhanced_segmentation.py for other strategies (demo mode)
        # Note: enhanced_segmentation.py doesn't accept command line args in current version
        logger.info(f"ℹ️ Running enhanced segmentation demo (strategy: {args.strategy})")
        return run_script(SCRIPT_PATHS['segment'], [])  # no args for demo
...
def rag_prepare_command(args) -> int:
    ...
    # Note: The RAG script doesn't have an --output parameter, 
    # it generates output files automatically based on input filename
    logger.info("ℹ️ Output files will be generated automatically based on input filename")
