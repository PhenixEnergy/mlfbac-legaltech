...
def _convert_search_engine_to_api_result(engine_result) -> SearchResult:
    ...
    chunk_metadata = ChunkMetadataResponse(
        chunk_id=engine_result.chunk_id,
        source_gutachten_id=engine_result.source_gutachten_id,
        level=engine_result.metadata.get('level', 1),
        section_type=engine_result.section_type,
        start_char=engine_result.metadata.get('start_char', 0),
        end_char=engine_result.metadata.get('end_char', 0),
        token_count=engine_result.metadata.get('token_count', 0),
        semantic_score=engine_result.similarity_score,
        relevance_score=engine_result.relevance_score,
        legal_norms=engine_result.legal_norms or [],
        keywords=engine_result.keywords or []
    )
    
    # Create API result
    return APISearchResult(
        chunk_id=engine_result.chunk_id,
        content=engine_result.text,
        similarity_score=engine_result.similarity_score,
        metadata=chunk_metadata,
        gutachten_metadata=None  # Could be enhanced later
    )
...
def _convert_search_engine_to_api_response(engine_response, request_query: str, search_time_ms: float):
    ...
    api_results = [
        _convert_search_engine_to_api_result(result) 
    ...
    return APISearchResponse(
        ...
        legal_areas_found=engine_response.aggregations.get('legal_areas', {}),
        legal_norms_found=engine_response.aggregations.get('legal_norms', {}),
        avg_similarity_score=engine_response.avg_similarity_score
    )
...
async def semantic_search(
    ...
    try:
        ...
        if request.sort_by.value == "similarity":
            ...
            # For now, skip date sorting since we don't have gutachten_metadata in results
            pass
        # 'relevance' ist bereits Standard-Sortierung
        
        # Convert to API models
        api_search_time = (time.time() - start_time) * 1000
        response = _convert_search_engine_to_api_response(
            search_results, 
            request.query, 
            api_search_time
        )
        
        logger.info(f"Search completed: {len(response.results)} results in {response.search_time_ms:.2f}ms")
        return response
